...
export async function getStaticProps() {
  ...
  // grab the fields
  const newDelights = delightRes.items.map((delight: any) => ({
    ...delight.fields,
    id: delight.sys.id,
  }));
...
export default function shopPage({ delights }: { delights: Delights[] }) {
  // write to db delights data
  useEffect(() => {
    const writeDB = async () => {
      await fetch('/api/writeDB', {
        method: 'POST',
        body: JSON.stringify(delights),
      });
    };
              ...
              <div className="ml-auto pl-[1rem] w-full sm:w-3/4 ">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[2rem]">
                  {delights.map((delight) => {
                    if (
                      typeValue.includes(delight.delightType[0].toLowerCase()) &&
                      delight.price < sliderValue
                    ) {
                      return <DelightCard key={delight.id} {...delight} />;
                    }
                  })}
                </div>
              </div>
